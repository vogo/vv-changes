...
vv/
  ...
  dispatch/          -- Orchestration dispatch (extracted from orchestrator.go)
    ...
    explore.go       -- explore, exploreStream logic
    classify.go      -- classify, classifyStream, classifyDirect logic
...
main.go
  --> setup/
      --> config/          (unchanged)
      --> registry/        (depends on: vage/tool, vage/agent, config/)
...
//
// This approach avoids assuming tool.Registry supports filtering or enumeration.
// Instead, it calls the individual tool registration functions directly,
// matching the pattern used in tools/tools.go.
...
**Design decisions:**
- `BuildRegistry` constructs a new registry from scratch by calling individual tool registration functions, because `tool.Registry` does not expose filtering or enumeration APIs. This approach preserves tool configuration (bash timeout, working dir) and matches the existing pattern in `tools/tools.go`.
- The three separate `tools.Register*()` functions become redundant and can be removed or simplified once this is in place.
- `ProfileByName` provides a clean mapping from JSON string values (used in `DynamicAgentSpec.ToolAccess`) to `ToolProfile` values.
...
- Both `Register` (returns error) and `MustRegister` (panics) are provided. `MustRegister` is used in `setup.go` where a duplicate is a programming error; `Register` is available for testing and future dynamic registration.
...
- `PersistentMemory` is explicitly part of `FactoryOptions` to support the coder's persistent memory prompt without special-casing in setup logic.
...
**File: `vv/dispatch/explore.go`**

Exploration logic extracted from `orchestrator.go` methods: `explore`, `exploreStream`.
...
Classification logic extracted from `orchestrator.go` methods: `planTask`, `planTaskStream`, `classifyTaskDirect`.
...
type DynamicAgentSpec struct {
    ...
    ToolAccess   string `json:"tool_access,omitempty"`   // profile name string for JSON compat
    Model        string `json:"model,omitempty"`
}

// validate checks that a DynamicAgentSpec is well-formed.
// Uses registry to validate base_type and ProfileByName to validate tool_access.
...
// runDirect dispatches to a single sub-agent, with lifecycle hooks.
...
// Each node's Runner is wrapped with lifecycle hooks via wrapWithHooks.
...
// Uses ToolProfile.BuildRegistry(d.toolsCfg) to create the filtered tool registries.
...
// This enables hooks to fire for both direct dispatch and DAG-executed agents.
...
func (h *hookedAgent) ID() string
func (h *hookedAgent) Name() string
func (h *hookedAgent) Description() string
```

**Key changes in `buildDynamicAgent`**: Instead of using `defaultBaseTypeToolAccess` and `defaultBaseTypePrompt` maps plus the special `reviewReg` field, the method now:
1. Looks up the `AgentDescriptor` from the registry by `spec.BaseType`
2. Gets the `ToolProfile` from the descriptor (or resolves `spec.ToolAccess` string via `ProfileByName()`)
3. Calls `profile.BuildRegistry(d.toolsCfg)` to create the filtered tool registry from scratch
...
**Key change in `buildNodes`**: Each `orchestrate.Node.Runner` is wrapped via `wrapWithHooks()` so that lifecycle hooks fire for DAG-executed agents, not just directly dispatched agents.
...
func RegisterReviewer(reg *registries.Registry) {
    reg.MustRegister(registries.AgentDescriptor{
        ...
        ToolProfile:  registries.ProfileReview,   // unified model, no special-casing
        ...
        Factory: func(opts registries.FactoryOptions) (agent.Agent, error) {
            ...
            return taskagent.New(
                agent.Config{ID: "reviewer", Name: "Reviewer Agent", Description: "Reviews code"},
                taskOpts...,
            ), nil
        },
    })
...
**Special cases:**
- **Coder with persistent memory prompt**: The `RegisterCoder` factory checks `opts.PersistentMemory` and constructs a `PersistentMemoryPrompt` when available, preserving the current behavior where the persistent prompt replaces the static `CoderSystemPrompt`.
...
**Hook integration in Dispatcher**: Hooks are applied at two points:

1. **Direct dispatch** (`runDirect`, `forwardSubAgentStream`): Via `runWithHooks` helper:
```go
...
2. **DAG execution** (`buildNodes`): Each `orchestrate.Node.Runner` is wrapped with `wrapWithHooks()` which returns a `hookedAgent` that invokes hooks around `Run` calls. This ensures hooks fire for DAG-executed agents without modifying the `vage` orchestrate framework.
...
type Result struct {
    ...
    registry   *registries.Registry          // unexported; use Agents() for HTTP registration
    subAgents  map[string]agent.Agent       // unexported; use Agents() for HTTP registration
}

// Agents returns the dispatchable agents suitable for HTTP service registration.
func (r *Result) Agents() []agent.Agent {
    agents := make([]agent.Agent, 0, len(r.subAgents))
...
type Options struct {
    WrapToolRegistry func(*tool.Registry) *tool.Registry // optional: wraps tool registries (e.g., CLI confirmation)
}
...
func New(
    ...
    for _, desc := range reg.Dispatchable() {
        ...
        if opts != nil && opts.WrapToolRegistry != nil {
            toolReg = opts.WrapToolRegistry(toolReg)
        }
    ...
    dispatcher := dispatch.New(
        ...
        dispatch.WithMaxConcurrency(cfg.Orchestrate.MaxConcurrency),
...
**Design decisions on `main.go` changes**: The `main.go` simplification:
- Creates a single `fullToolReg` (all 6 tools) using `tools.Register()` for the HTTP service registration -- same as today
- Calls `setup.New()` instead of `agents.Create()`
- Passes `WrapToolRegistry` option for CLI confirmation wrapping: `&setup.Options{WrapToolRegistry: func(r *tool.Registry) *tool.Registry { return vvcli.WrapRegistry(r, cfg.CLI.ConfirmTools) }}`
- Uses `result.Dispatcher` for CLI and HTTP modes
- Uses `result.Agents()` for HTTP service agent registration
...
| `hookedAgent` | `dispatch` | `inner agent.Agent`, `hooks []lifecycle.Hook`, `agentID string` | Hook-wrapped agent for DAG nodes |
...
| `validToolAccessLevels` map | Replaced by `ProfileByName()` |
...
|--------|---------|-------|
| `main.go` | `agents.Create(cfg, llm, coderReg, readOnlyReg, reviewReg, memMgr, prompt)` | `setup.New(cfg, llm, memMgr, persistentMem, opts)` |
| `main.go` | `allAgents.Orchestrator` | `result.Dispatcher` |
| `main.go` | `allAgents.Coder`, `allAgents.Chat`, etc. | `result.Agent("coder")`, `result.Agent("chat")`, etc. |
...
- Create `vv/registry/tool_access.go` with `ToolCapability`, `ToolProfile`, predefined profiles, `ProfileByName()`, `BuildRegistry()`, `registerCapabilityTools()`
- Create `vv/registry/registries.go` with `Registry`, `AgentDescriptor`, `AgentFactory`, `FactoryOptions`, `MustRegister()`, `Register()`
...
- Create `vv/dispatch/dispatch.go` -- `Dispatcher` struct, `New()` with functional options, `Run()`, `RunStream()`
- Create `vv/dispatch/explore.go` -- move `explore`, `exploreStream`
- Create `vv/dispatch/classify.go` -- move `planTask`/`classify`, `planTaskStream`/`classifyStream`, `classifyTaskDirect`/`classifyDirect`
- Create `vv/dispatch/dag.go` -- move `buildNodes`, `buildDynamicAgent`, `runPlan`, `runDirect`, `findTerminalNodes`, `wrapWithHooks`, `hookedAgent`
...
- Replace `toolRegistries` map and `reviewReg` with `ToolProfile.BuildRegistry(d.toolsCfg)`
- Use `ProfileByName()` for `spec.ToolAccess` string resolution
...
- Use `ProfileByName()` in `DynamicAgentSpec.validate()`

**Task 2.5: Add hook wrapping to `buildNodes`**
- Wrap each `orchestrate.Node.Runner` with `wrapWithHooks()` for DAG-executed agents

**Task 2.6: Migrate orchestrator tests**
...
- Refactor `agents/coder.go`: `newCoderAgent()` -> `RegisterCoder(reg)` with `PersistentMemory` support in factory
...
- Create `vv/setup/setup.go` with `New()` function, `Result` struct, `Options` struct
- Implement `Agents()` and `Agent(id)` methods on `Result`
- Implement proper error handling for all agent construction
...
- Pass `WrapToolRegistry` option for CLI confirmation
- Update HTTP service agent registration to use `result.Agents()`
...
- Verify tool counts per profile: full=6, read-only=3, review=4, none=0
...
|------|---------|-----------|
| `TestToolProfile_BuildRegistry_Full` | `registry` | Full profile constructs registry with all 6 tools |
| `TestToolProfile_BuildRegistry_ReadOnly` | `registry` | ReadOnly profile constructs registry with read, glob, grep (3 tools) |
| `TestToolProfile_BuildRegistry_Review` | `registry` | Review profile constructs registry with bash, read, glob, grep (4 tools) |
...
| `TestProfileByName` | `registry` | All profile names resolve correctly; unknown names return false |
| `TestRegistry_MustRegister_Get` | `registry` | Register and retrieve a descriptor |
| `TestRegistry_Register_Error` | `registry` | Register returns error on duplicate ID |
| `TestRegistry_MustRegister_Duplicate_Panics` | `registry` | Duplicate ID panics |
...
| `TestHookedAgent_Run` | `dispatch` | Hooks fire around wrapped agent execution |
...
| `TestDynamicAgentSpec_Validate` | `dispatch/types_test.go` | Use registry + `ProfileByName()` |
| `TestCreate_AllAgents` | `setup/setup_test.go` | Use `setup.New()` and verify all agents created |
| `TestCreate_Names` | `setup/setup_test.go` | Verify agent names via `result.Agent()` |
...
5. **Dynamic agent creation**: `buildDynamicAgent` produces agents with same tool access and prompts via registry lookup + `BuildRegistry(toolsCfg)`
...
8. **CLI confirmation preserved**: `WrapToolRegistry` option ensures coder tools still require CLI confirmation when configured
9. **Persistent memory prompt preserved**: Coder factory constructs `PersistentMemoryPrompt` when `PersistentMemory` is provided
10. **Lifecycle hooks fire for DAG agents**: `hookedAgent` wrapper ensures hooks are invoked for both direct and DAG dispatch paths
